...
time_span = datenum(year,01,01):datenum(year,12,31);
...
parfor ff = 1:length(file_list)
    ...
    tem_sst_eo = sst_eo(:,:,ff);
    tem_sst_eo2 = tem_sst_eo(mask(:,:,1) == 1);

    % Build interpolant
    ft = scatteredInterpolant(double(lonm), double(latm), tem_sst_eo2, 'natural', 'linear');

    sst(:,ff) = ft(fvcomlon, fvcomlat);
    time(ff) = time_out(ff) + 0.5; % fvcom expects these to be at mid-day
end
