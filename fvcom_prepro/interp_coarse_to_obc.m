...
[lon, lat] = deal(coarse.lon, coarse.lat);

% Find the indices of all the coarse data which cover the open boundary
...
for obc_i = 1:length(oNodes)
    ...
    obc_coarse_idx = [obc_coarse_idx; h_idx(1:20)];
end
...
obc_coarse_idx = unique(obc_coarse_idx, 'stable');
[xidx, yidx] = ind2sub(size(lon), obc_coarse_idx);
...
% Make a 3D array of the coarse depths and mask where we don't have data.
% This can then be used in the interpolation instead of trying to deal with
% this on the fly.
hdepth = permute(repmat(coarse.Depth.data, [1, nx, ny]), [2, 3, 1]);
mask = coarse.(fields{ff}).data(:, :, :, 1) > 1.26e29;
...
% removal.
% hdepth(mask) = nan;
% landmask = coarse.(fields{ff}).data(:, :, 1, 1) > 1.26e29;

% Ignore depth layers in the coarse data which are below the maximum depth
...
for v = 1:length(fields)
    ...
    if ~(isfield(coarse.(fields{v}), 'data') && ndims(coarse.(fields{v}).data) > 3)
        continue
    end
    ...
    for t = 1:nt
        ...
        for j = 1:nz
            ...
            switch fields{v}
                ...
                case 'ssh'
                    ...
                    if min(mask_alt(:)) == 1 && warned(3)
                        warned(3) = false;
                        warning('Removing sea surface height values below -20m from the coarse data.')
                    end
                case {'u', 'v'}
                    mask_alt = tpctemp2 > 100 | tpctemp2 < -100 ;
                    if min(mask_alt(:)) == 1 && warned(4)
                        warned(3) = false;
                        warning('Removing non-tidal velocities above/below +/-100m/s from the coarse data.')
                    end
                otherwise
            ...
            % region.
            tlon = lon(obc_coarse_idx);
            tlat = lat(obc_coarse_idx);
            tlon(mask) = [];
            tlat(mask) = [];

            % If the coarse depths are now below the maximum depth in the
            ...
            parfor i = 1:nf
                ...
                % Get the n nearest positions from coarse data (more?
                % fewer?).
                np = 16;
                if length(ii) < np
                ...
                % the coarse grid resolution, skip this point at this
                ...
                % values. If the triangulation fails (when the points are
                % in a line), do an inverse distance weighed interpolation
                % instead
                if length(plon) < 3
                ...
                % if i == 1
                %     clf
                %     pcolor(coarse.lon - (hdx/2), coarse.lat - (hdx/2), coarse.(fields{v}).data(:, :, j, t)); colorbar; hold on
                %     shading flat
                %     hold on
                % end
                ...
                if isnan(itempobc(i))
                    ...
                    if j == 1
                        ...
                        %clf
                        %pcolor(coarse.lon, coarse.lat, landmask * 1); colorbar; hold on
        ...
        % zz = repmat(-coarse.Depth.data, [1, nf])';
        % dd = nanmax(hdepth, [], 3);
        % cc = itempz;
        % mm = ~isnan(cc);
        ...
        % hold on
        % scatter3(coarse.lon(:), coarse.lat(:), -dd(:), 40, 'c.')
        ...
        for pp = 1:nf
            ...
            % The coarse data is unusual in that the depths are fixed and
            ...
            % Find the coarse depths which cover the modelled depth range.
...
for s = 1:length(fvfields)
    switch fvfields{s}
        ...
        otherwise
            Mobj.(fvfields{s}) = fvcom.(fvfields{s});
...
% distribution of interpolated values over the coarse data. Add the location
% of the vertical profile (both FVCOM and coarse) to the plot.
...
%
% subplot(2,2,2)
% plot(squeeze(coarse.temperature.data(xidx, yidx, zidx, tt)), squeeze(-hdepth(xidx, yidx, zidx)), 'rx-')
% xlabel('Temperature (^{\circ}C)')
% ylabel('Depth (m)')
% title('coarse')
%
...
%
% subplot(2,2,4)
% plot(squeeze(coarse.temperature.data(xidx, yidx, zidx, tt)), hz(zidx), 'rx-')
% xlabel('Temperature (^{\circ}C)')
% ylabel('Array index')
% title('coarse')
%
% % Figure to check everything's as we'd expect. Plot first time step with
% % the coarse surface temperature as a background with the interpolated
% % boundary node surface values on top.
%
% figure(200)
% clf
% % Plot coarse surface data (last sigma layer)
% dx = mean(diff(coarse.lon(:)));
% dy = mean(diff(coarse.lat(:)));
% temp = coarse.temperature.data(:, :, :, tt);
% pcolor(coarse.lon - (dx / 2), coarse.lat - (dy / 2), ...
%     squeeze(temp(:, :, hyz)))
% shading flat
% axis('equal', 'tight')
% daspect([1.5, 1, 1])
